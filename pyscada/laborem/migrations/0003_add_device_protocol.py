    ...
    if not DeviceProtocol.objects.using(db_alias).filter(pk=PROTOCOL_ID):
        DeviceProtocol.objects.using(db_alias).bulk_create([
            DeviceProtocol(pk=PROTOCOL_ID,
                           protocol='laborem',
                           description='LaboREM Interface',
                           app_name='pyscada.laborem',
                           device_class='pyscada.laborem.device',
                           daq_daemon=True,
                           single_thread=True),
        ])
...
def reverse_func(apps, schema_editor):
    ...
    DeviceProtocol = apps.get_model("pyscada", "DeviceProtocol")
    db_alias = schema_editor.connection.alias
    DeviceProtocol.objects.using(db_alias).filter(protocol="laborem").delete()


class Migration(migrations.Migration):
    dependencies = [
        ('laborem', '0002_extendedlaboremmotherboarddevice'),
